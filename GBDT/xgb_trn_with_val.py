...
# parser.add_argument('--mcw', type=int, default=5)
...
if args.train:
    ...
    log_path = f"./log/XGBoost/{args.probs}-all-trn_with_val/{log_time}.log"
...
if args.probs == 'all' or args.probs == 'install':
    cache_path = '/root/autodl-tmp/xingmei/RecSys23/data/install_probs_tvt_0.cache'
...
if args.train:
    ...
    val_d = xgb.DMatrix(val_X, enable_categorical=ec, feature_names=list(val_X.columns),  feature_types=ft)
    val_logloss = 0
    if args.fold is not None:
        ...
        for i, (trn_idx, tst_idx) in enumerate(kf.split(trn_df)):
            ...
            trn_d = xgb.DMatrix(trn_X, trn_y, enable_categorical=ec,  feature_names=list(trn_X.columns),  feature_types=ft)
            tst_d = xgb.DMatrix(tst_X, tst_y, enable_categorical=ec, feature_names=list(trn_X.columns),  feature_types=ft)
            model = xgb.train(
                        params, 
                        trn_d, 
            ...
            val_logloss += M.log_loss(val_y, model.predict(val_d))
        ...
        logger.info(f'Best score: {model.best_score} | Log time {log_time} | {M.log_loss(val_y, preds)}')
